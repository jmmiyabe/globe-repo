...
import { AlertTriangle, Cloud, Shield, Radio, AlertCircle, Users, MapPin } from "lucide-react"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {keyMetrics.map((metric, idx) => (
            <Card key={idx} className={`border ${getMetricColor(metric.color)}`}>
              ...
              <CardContent>
                <div className={`text-3xl font-bold ${getMetricTextColor(metric.color)}`}>{metric.value}</div>
          ...
          <CardContent>
            <div className="space-y-4">
              {urgentAlerts.map((alert) => (
                <div
                  key={alert.id}
                  className={`p-4 rounded-lg border flex items-start justify-between gap-4 ${getAlertColor(alert.type)}`}
                >
                  ...
                  {alert.action && (
                    <Button
                      ...
                      onClick={() => console.log(`[v0] Action: ${alert.action}`)}
                    >
                      {alert.action}
                    </Button>
                  )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="border-border lg:col-span-2">
            <CardHeader>
              ...
              <CardDescription>Historical alert levels over the last 24 hours</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={responseMetrics} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                  ...
                  />
                  <Bar dataKey="critical" stackId="a" fill="var(--color-status-critical)" name="Critical" />
                  <Bar dataKey="warning" stackId="a" fill="var(--color-status-warning)" name="Warning" />
                  <Bar dataKey="safe" stackId="a" fill="var(--color-status-safe)" name="Safe" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
